...
2.  **Generate Candidates:** Brainstorm 3-5 distinct approaches.
    - _Constraint:_ Ensure **Diversity** (NQD). Include at least one "Conservative" (safe) and one "Radical" (novel) option.
...
3.  Call `quint_propose` for EACH hypothesis.
    - _Note:_ The tool will store these in **`.quint/knowledge/L0/`**.
...
- **content**: The Method (Recipe). Detail _how_ it works.
- **scope**: The Claim Scope (G). Where does this apply?
  - _Example:_ "High-load systems, Linux only, requires 1GB RAM."
- **kind**: "system" (for code/architecture) or "episteme" (for process/docs).
- **rationale**: A JSON string explaining the "Why".
  - _Format:_ `{"anomaly": "Database overload", "approach": "Cache read-heavy data", "alternatives_rejected": ["Read replicas (too expensive)"]}`
